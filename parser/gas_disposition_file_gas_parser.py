'''
@File   :  gas_disposition_file_gas_parser.py
@Author :  Haitao Lu
@Date   :  25-Mar-2023 4:38 AM
...
from layouts.gas_disposition_file_gas import GAS_DISPOSITION_GAS_LAYOUT


def gas_disposition_file_gas_parser(source_file_path, dest_folder_path='', block_size=200, limiting_counter=False,
                                    check_stop=100):
	'''
	Extract data from the oil ledger files based on layout in gas_disposition_file_oil.py
	Saves the data to a csv file

	:param source_file_path: path to the gas_disposition_file_gas file
	...
	for block in yield_blocks(file, block_size):  # for each block in file
		...
		parsed_vals = parse_record(block, GAS_DISPOSITION_GAS_LAYOUT)  # formats the record and returns a formated {dict}
...
if __name__ == '__main__':
	file_path = r'/Volumes/haitao_folder/rrc/PR_Gas_Disposition/gsf102.ebc'
	gas_disposition_file_gas_parser(file_path)
